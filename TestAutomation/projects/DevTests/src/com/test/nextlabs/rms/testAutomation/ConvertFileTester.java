...
public class ConvertFileTester {
	
	public String inputFolder = IntegrationTester.inputDir+File.separator+TestingConfigManager.CONVERT_FILE_FOLDER_NAME;
	private String report = "";		

	public void sendFileConvertRequest() {
		int totTestCases = 0;
		...
		try{			
			...
			for (File file : dir.listFiles())
			{
				...
				if(!inputFileName.endsWith("_RESPONSE.xml")){
				   ...
				   String responseString = HttpRequestUtil.sendPostRequest(encoded,"ConvertFile",headerMap);
		    ...
		    setReport("ConvertFile test results:" + "\n");
			...
			e.printStackTrace();			
			System.out.println("ConvertFile test failed with an Exception");
			System.out.println("Test cases executed: "+ totTestCases);
		    System.out.println("Passed: " + (totTestCases-numOfFailedCases));
		    System.out.println("Failed: " + numOfFailedCases);
		    
		    setReport("ConvertFile test failed with an Exception:" + "\n");
		    setReport(getReport() + "    Test cases executed: "+ totTestCases +"\n");
		    setReport(getReport() + "    Passed: " + (totTestCases-numOfFailedCases)+"\n");
		    setReport(getReport() + "    Failed: " + numOfFailedCases +"\n");
		    if(failedCases.size() > 0){
