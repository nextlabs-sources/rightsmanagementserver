...
class Installation < Shoes
  ...
  url '/icenet_server',             :icenet_server
      ...
      stack  @@installer_content_size_style do
        background @@content_color_style
         stack :width => 1.0, :height => 40 do
           para ReadableNames["icenet_server_page"]["heading"], @@heading_1_style 
         end
         stack :width => 1.0, :height => 60, :margin_right => 10 do
          para ReadableNames["icenet_server_page"]["help_note"],  @@help_note_style
         end
        flow :width => 1.0, :height => 40 do 
           stack :width => 200, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 30 do
              para ReadableNames["inputs"]["icenet_server"],  @@text_label_style
           end
           stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
            @icenet_server_edit = nl_text_edit( :long, text=$item.icenet_server, { :width => 300 } )
			@icenet_server_edit.focus
           end
		   stack :width => 300, :margin_left => 20, :margin_right => 10, 
               :margin_top => 20, :height => 30 do
              para ReadableNames["icenet_server_page"]["icenet_server_hint"],  @@text_hint_style
           end
        end 
      end
      ...
      stack @@installer_footer_size_style do
        ...
        flow :width => 1.0, :height => 50 do
          ...
          stack :width => 50, :height => 1.0 do
            @icenet_server_back_btn = nl_button :back
            @icenet_server_back_btn.click { icenet_back_proc.call }
          end
          stack :width => 50, :height => 1.0 do
            @icenet_server_back_text_btn = nl_button :back_text, :click => icenet_back_proc
          end
          ...
          icenet_next_proc = Proc.new{
			control_fields = {
              "icenet_server"      => @icenet_server_edit,
            }
            $item.icenet_server = @icenet_server_edit.text
			validated, field, error_msg = $item.validate_fields "icenet_server"
	
            if !validated
              alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
              control_fields[field].focus
			elsif !Utility::URLValidator.is_uri_valid? $item.icenet_server
		   	  alert_ontop_parent(app.win, "#{$item.icenet_server} is not a valid URL.", :title => app.instance_variable_get('@title'))
			  control_fields["icenet_server"].focus
			else
			  visit(wizard(:next))
			end
          }
          ...
          stack :width => 50, :height => 1.0 do
            @icenet_server_next_text_btn = nl_button :next_text, :click => icenet_next_proc
          end

          stack :width => 50, :height => 1.0 do
            @icenet_server_next_btn = nl_button :next
            @icenet_server_next_btn.click { icenet_next_proc.call }
          end
		  keypress { |k| 
			icenet_next_proc.call if k == "\n"
		  }
        end
      end
    end
