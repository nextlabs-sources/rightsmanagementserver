...
class Installation < Shoes
  ...
  url '/kms_db_config',             :kms_db_config
  ...
  def kms_db_config
    $item.kms_db_type  = Item::DB_FRIENDLY_NAMES_REVERSE_MAP.keys[0] if $item.kms_db_type ==""
	$item.kms_db_port = Item::DB_PORT[$item.kms_db_type] if $item.kms_db_port==""
      ...
      stack  @@installer_content_size_style do
        ...
        background @@content_color_style
         ...
         stack :width => 1.0, :height => 40 do
           para ReadableNames["db_config_page"]["heading_kms"], @@heading_1_style 
         end

         stack :width => 1.0, :height => 60, :margin_right => 10 do
          para ReadableNames["db_config_page"]["help_note_kms"],  @@help_note_style
         end
		 
          flow :width => 1.0, :height => 40 do
            stack :width => 200, :margin_left => 10, :margin_right => 10, 
                 :margin_top => 20, :height => 30 do
                para ReadableNames["inputs"]["kms_db_type"],  @@text_label_style
            end 
            stack :width => 300, :margin_left => 10, :margin_right => 10, 
              ...
              my_list_box = list_box(items: Item::DB_FRIENDLY_NAMES_MAP.keys,
                    width: 200, choose: Item::DB_FRIENDLY_NAMES_REVERSE_MAP[$item.kms_db_type])     			
              ...
              @db_type_select = $item.kms_db_type			  
            end
			
			@in_built_port_flow = flow :width => 1.0, :height => 40 do      
				stack :width => 200, :margin_left => 10, :margin_right => 10, 
				   :margin_top => 20, :height => 30 do
				  para ReadableNames["inputs"]["kms_db_port"],  @@text_label_style
				end
				stack :width => 300, :margin_left => 10, :margin_right => 10, 
				   :margin_top => 20, :height => 40 do
				  @in_built_db_port_edit = nl_text_edit( :short, text = $item.kms_db_port, { :width => 300 } )  
				end	
			end
			@in_built_port_flow.hide()
			@in_built_password_flow = flow :width => 1.0, :height => 40 do      
				stack :width => 200, :margin_left => 10, :margin_right => 10, 
					:margin_top => 20, :height => 30 do
					para ReadableNames["inputs"]["kms_db_password"],  @@text_label_style
				end
				stack :width => 300, :margin_left => 10, :margin_right => 10, 
					:margin_top => 20, :height => 40 do
					@in_built_db_password_edit = nl_text_edit( :short, text=$item.kms_db_password, { :width => 300, :secret => true } )
				end      
        ...
        @db_host_name_flow = flow :width => 1.0, :height => 40 do      
           stack :width => 200, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 30 do
              para ReadableNames["inputs"]["kms_db_host_name"],  @@text_label_style
           end
            stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
              @db_host_name_edit = nl_text_edit( :short, text=$item.kms_db_host_name, { :width => 300 } )
        ...
        @db_port_flow = flow :width => 1.0, :height => 40 do      
           stack :width => 200, :margin_left => 10, :margin_right => 10, 
            ...
            stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
              @db_port_edit = nl_text_edit( :short, text = $item.kms_db_port, { :width => 300 } )  
            end
        ...
        @db_name_flow = flow :width => 1.0, :height => 40 do 
           stack :width => 200, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 30 do
              para ReadableNames["inputs"]["kms_db_name"],  @@text_label_style
           end
           stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
            @db_name_edit = nl_text_edit( :short, text=$item.kms_db_name, { :width => 300 } )
        ...
        @db_username_flow = flow :width => 1.0, :height => 40 do 
           stack :width => 200, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 30 do
              para ReadableNames["inputs"]["kms_db_username"],  @@text_label_style
           end
           stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
            @db_username_edit = nl_text_edit( :short, text=$item.kms_db_username, { :width => 300 } )
           end      
        end  
        ...
        @db_password_flow = flow :width => 1.0, :height => 40 do 
           ...
           stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
            @db_password_edit = nl_text_edit( :short, text=$item.kms_db_password, { :width => 300, :secret => true } )
           end      
        end
		...
		@db_conn_url_flow = flow :width => 1.0, :height => 40 do	
           stack :width => 200, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 30 do
              para ReadableNames["inputs"]["kms_db_conn_url"],  @@text_label_style
           end
           stack :width => 300, :margin_left => 10, :margin_right => 10, 
               :margin_top => 20, :height => 40 do
            @db_conn_url_edit = nl_text_edit( :short, text=$item.kms_db_conn_url, { :width => 300 })
           end      
        end 		
        ...
        db_config_testcon_proc = Proc.new{ 
            control_fields = {                                  
              "kms_db_host_name" => @db_host_name_edit,
              "kms_db_port" => @db_port_edit,
              "kms_db_name" => @db_name_edit,
              "kms_db_username" => @db_username_edit,
              "kms_db_password" => @db_password_edit        
            }

            validated, field, error_msg = $item.validate_fields "kms_db_host_name", "kms_db_port", "kms_db_name"
            if !validated && @db_conn_url_edit.text == ""
              ...
              control_fields[field].focus
				...
				error_msg = ReadableNames["ErrorMsgTemplate"] % [ReadableNames["inputs"]["kms_db_username"], ReadableNames["validator_errors"]["non_empty"]]
				alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
          ...
          db_config_next_proc = Proc.new{            
			...
			if @db_type_select != "IN_BUILT"
              ...
              if !validated && @db_conn_url_edit.text == ""
				...
				validated, field, error_msg = $item.validate_fields "kms_db_username"
				if !validated
					alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
					control_fields[field].focus
				else
			  ...
			  if !Utility::TCP.is_valid_port($item.kms_db_port)
				error_msg = ReadableNames["ErrorMsgTemplate"] % [ReadableNames["inputs"]["kms_db_port"], ReadableNames["validator_errors"]["port"]]
				alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
				@in_built_db_port_edit.focus
			  elsif $item.kms_db_port != $item.rms_db_port
				error_msg = ReadableNames["db_config_page"]["in_built_diff_port_err"] % [$item.rms_db_port]
			    alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
				@in_built_db_port_edit.focus
			  elsif !Utility::TCP.is_port_available($item.kms_db_port)
				alert_ontop_parent(app.win, "Port #{$item.kms_db_port} is not available. Please choose another port.", :title => app.instance_variable_get('@title'))
				@in_built_db_port_edit.focus
			  elsif @in_built_db_password_edit.text == ""
				error_msg = ReadableNames["ErrorMsgTemplate"] % [ReadableNames["inputs"]["kms_db_password"], ReadableNames["validator_errors"]["non_empty"]]
				alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
				@in_built_db_password_edit.focus
			  elsif $item.kms_db_password != $item.rms_db_password
			    error_msg = ReadableNames["db_config_page"]["in_built_diff_password_err"]
				alert_ontop_parent(app.win, error_msg, :title => app.instance_variable_get('@title'))
				@in_built_db_password_edit.focus
			  else
				visit(wizard(:next))
