...
import com.nextlabs.kms.GetKeyRingsRequest;
import com.nextlabs.kms.GetKeyRingsResponse;
...
public class GetKeyRingsResource extends ServerResource {
	...
	public Representation doPost(Representation entity) throws IOException {	
		...
		GetKeyRingsResponse keyRingResponse = factory.createGetKeyRingsResponse();
		GetKeyRingsRequest keyRingRequest = JAXB.unmarshal(new StringReader(entity.getText()), GetKeyRingsRequest.class);
		String responseXml = "";	
		try {
			...
			for (IKeyRing iKeyRing: iKeyRings) {
				KeyRingDTO keyRing = DTOConverter.convertToKeyRingDTO(iKeyRing);
				keyRingResponse.getKeyRings().add(keyRing);
			}
