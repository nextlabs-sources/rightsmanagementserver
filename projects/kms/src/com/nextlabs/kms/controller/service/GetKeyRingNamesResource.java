...
import com.nextlabs.kms.GetKeyRingNamesRequest;
...
public class GetKeyRingNamesResource extends ServerResource {
	...
	public Representation doPost(Representation entity) throws IOException {	
		...
		GetKeyRingNamesResponse keyRingResponse = factory.createGetKeyRingNamesResponse();
		GetKeyRingNamesRequest keyRingRequest = JAXB.unmarshal(new StringReader(entity.getText()), GetKeyRingNamesRequest.class);
		String responseXml = "";	
		try {
			...
			Set<String> keyRingNames = keyRingManager.getKeyRingNames();
			KeyRingNamesDTO keyRingNamesDTO = new KeyRingNamesDTO();
			keyRingNamesDTO.getName().addAll(keyRingNames);
			keyRingResponse.setKeyRingNames(keyRingNamesDTO);
