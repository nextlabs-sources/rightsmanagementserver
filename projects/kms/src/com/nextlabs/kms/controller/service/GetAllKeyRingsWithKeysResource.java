...
public class GetAllKeyRingsWithKeysResource extends ServerResource {
	...
	public Representation doPost(Representation entity) throws IOException {	
		...
		GetAllKeyRingsWithKeysResponse keyRingResponse = factory.createGetAllKeyRingsWithKeysResponse();
		GetAllKeyRingsWithKeysRequest keyRingRequest = JAXB.unmarshal(new StringReader(entity.getText()), GetAllKeyRingsWithKeysRequest.class);
		String responseXml = "";	
		try {
			...
			List<IKeyRing> iKeyRings = keyRingManager.getKeyRings();
			KeyRingsWithKeysDTO keyRings = DTOConverter.convertToKeyRingsWithKeysDTO(iKeyRings);
			keyRingResponse.setKeyRingWithKeys(keyRings);
