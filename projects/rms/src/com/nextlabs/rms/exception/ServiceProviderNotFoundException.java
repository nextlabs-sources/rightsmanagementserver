/**
 * 
 */
package com.nextlabs.rms.exception;

/**
 * @author nnallagatla
 *
 */
public class ServiceProviderNotFoundException extends Exception {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4327062467459907147L;

	/**
	 * 
	 */
	public ServiceProviderNotFoundException() {
		
	}

	/**
	 * @param message
	 */
	public ServiceProviderNotFoundException(String message) {
		super(message);
	}

	/**
	 * @param cause
	 */
	public ServiceProviderNotFoundException(Throwable cause) {
		super(cause);
	}

	/**
	 * @param message
	 * @param cause
	 */
	public ServiceProviderNotFoundException(String message, Throwable cause) {
		super(message, cause);
	}

	/**
	 * @param message
	 * @param cause
	 * @param enableSuppression
	 * @param writableStackTrace
	 */
	public ServiceProviderNotFoundException(String message, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
	}

}
